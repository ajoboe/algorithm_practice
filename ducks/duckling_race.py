...
def run(ducks):
  ...
  event = Event(ducks, mother) # Quaaack! (translation: Go!)
  while mother not in event.ducks:
    event = getNextEvent(event)
  event.ducks.remove(mother)
  return event.ducks

def getNextEvent(prevEvent):
  nextBeginPassEvent = getNextBeginPassEvent(prevEvent)
  nextEndPassEvent = getNextEndPassEvent(prevEvent)
  if nextBeginPassEvent.time < nextEndPassEvent.time:
    return nextBeginPassEvent
  else: # nextBeginPassEvent.time > nextEndPassEvent.time: # change to else if later
    return nextEndPassEvent
#  else: # if nextBeginPassEvent.time == nextEndPassEvent.time:
#    if nextBeginPassEvent.pos == nextEndPassEvent.pos:
#      if

def getNextBeginPassEvent(prevEvent):
  # get list of ducks that are involved in events that happen at next time
  ducksInvolved = getDucksAtNextTime(prevEvent.ducks)
  ...
  timeOfThisEvent = prevEvent.time + shortestCatchupTime
...
def getDucksAtNextTime(ducks):
  ...
  for i in range(len(ducks) - 2):
    if catchupTime(i, ducks) == shortestCatchupTime:
      ducksInvolved.append(prevEvent.ducks[i])
